...
**`_news_days/YYYY-MM-DD.html`** — one thin Jekyll collection stub per digest date (front matter is just `digest_date: "YYYY-MM-DD"`, no duplicated data). The `news_days` collection (`_config.yml`) gives each date a real page at `/news/<date>/` via the `news-day` layout, which looks the actual digest back up from `site.data.generated.news[page.digest_date]`. This lets `/news/` inline only the latest day and lazy-load any other date over `fetch()` (see `assets/js/filters.js`), instead of shipping every digest ever generated in one HTML payload. Keep writing a stub alongside every `_data/generated/news/*.yml` file — `build_daily_digest` does this automatically. The 8 topic pages (`ai.md`, `security.md`, etc., via `_includes/topic-page.html`) reuse this same mechanism: they fetch `/news/<date>/` and filter its `[data-news-item]`s down to their own category client-side, so `_includes/news-day.html`'s `[data-news-day-body]` / `data-news-category` markers are a contract shared by both `/news/` and every topic page — don't rename them without updating `filters.js` in both places.
